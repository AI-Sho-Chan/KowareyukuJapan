...
$envFile = Join-Path $web '.env.staging'
$envExample = Join-Path $web '.env.staging.example'
if(-not (Test-Path $envFile)){
  if(Test-Path $envExample){
    Copy-Item -Path $envExample -Destination $envFile -Force
    Write-Host "Created $envFile from example. Please update values as needed."
  } else {
    throw 'Missing .env.staging and .env.staging.example. Please create one.'
  }
}

$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
# This runs: dotenv -e .env.staging -- cross-env NODE_ENV=production next build && next start -p 3031
$proc = Start-Process -FilePath 'npm.cmd' -ArgumentList 'run','preview:staging' -WorkingDirectory $web -WindowStyle Minimized -PassThru

$url = "http://localhost:$Port"
if(Wait-ForUrl -url $url -timeoutSec $TimeoutSec){
  Write-Host ("Staging preview is up at {0} (PID={1})" -f $url, $proc.Id)
  if(-not $NoBrowser){ Start-Process $url }
} else {
  Write-Warning ("Staging preview did not respond within {0}s." -f $TimeoutSec)
  exit 1
}
