...
$proc = Start-Process -FilePath 'npm.cmd' -ArgumentList 'run','dev' -WorkingDirectory $web -WindowStyle Minimized -PassThru

$url = "http://localhost:$Port"
if(Wait-ForUrl -url $url -timeoutSec $TimeoutSec){
  Write-Host ("Dev server is up at {0} (PID={1})" -f $url, $proc.Id)
  if(-not $NoBrowser){ Start-Process $url }
} else {
  Write-Warning ("Dev server did not respond within {0}s." -f $TimeoutSec)
  exit 1
}
