...
function getTweetId(u: string){
  try{ const m = new URL(u).pathname.match(/status(?:es)?\/(\d{5,})/); return m ? m[1] : ''; }catch{ return ''; }
}

function formatHandle(h?: string): string { const t=(h||'').trim(); return t ? (t.startsWith('@')?t:`@${t}`) : '@guest'; }
...
export default function XEmbedCard({ postId, title = '', comment, statusUrl, handle, tags, createdAt, adminHeader, footerExtras }: Props){
  const embedSrc = useMemo(() => {
    const id = getTweetId(statusUrl); if(!id) return '';
  ...
  return (
    <article className="card twitter-card" data-post-id={postId}>
      <div className="card-body">
        {adminHeader}
        <h2 className="title">{title || 'X / Twitter'}</h2>
