...
import { AutoTopic, loadTopics, saveTopics, readLogs } from '@/lib/auto-topics';
...
export async function GET(req: NextRequest) {
  ...
  // 最近のtopic_postログからキーワード別の投稿リストを生成
  const lines = readLogs(400);
  const recent: Record<string, Array<{ title: string; url: string }>> = {};
  for (const line of lines) {
    const m = line.match(/topic_post:\s*(.+?)\s*\|\s*(.+?)\s*\|\s*(https?:\/\/\S+)/);
    if (m) {
      const key = m[1];
      const title = m[2];
      const url = m[3];
      (recent[key] ||= []).push({ title, url });
    }
  }
  return NextResponse.json({ ok: true, topics, recent });
}

export async function POST(req: NextRequest) {
  if (!verifyAdminSession(req)) return NextResponse.json({ ok: false, error: 'unauthorized' }, { status: 401 });
  let body: any = {};
  try { body = await req.json(); } catch { try { const txt = await req.text(); body = JSON.parse(txt); } catch { body = {}; } }
