...
type Post = { id: string; title: string; url?: string; comment?: string; handle?: string; ownerKey?: string; createdAt?: number|string; is_published?: number };
...
export default function AdminConsole(){
  ...
  // Topics / logs (kept minimal)
  const [topics, setTopics] = useState<{ id:string; keyword:string; minIntervalMinutes:number }[]>([]);
  const [newTopic, setNewTopic] = useState('');
  const [newTopicMin, setNewTopicMin] = useState(60);
  const [logs, setLogs] = useState<string[]>([]);
  ...
  useEffect(()=>{
    (async()=>{
      ...
      setAuth(ok?'ok':'ng');
      if(ok){
        await Promise.all([reloadPosts(), reloadSummary(), reloadFlags(), reloadTopics(), reloadLogs()]);
      }
    })();
  },[]);
  ...
  async function login(e: React.FormEvent){
    e.preventDefault();
    const r = await fetch('/api/auth', { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ adminKey }) });
    if(r.ok){ setAuth('ok'); await Promise.all([reloadPosts(), reloadSummary(), reloadFlags(), reloadTopics(), reloadLogs()]); }
      ...
      <section className="card" style={{padding:12, marginTop:12}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 2fr auto', gap:8, marginBottom:8}}>
          ...
          <div style={{display:'flex', gap:10, alignItems:'center'}}>
            {['x','instagram','youtube','nhk','note','web'].map(s => (
              <label key={s} style={{fontSize:12}}>
                <input type="checkbox" checked={sourceFilter.includes(s)} onChange={e=>{
                  const checked = (e.currentTarget as HTMLInputElement).checked;
                  setSourceFilter(prev => checked ? Array.from(new Set([...prev, s])) : prev.filter(x => x !== s));
                }} /> {s}
              </label>
